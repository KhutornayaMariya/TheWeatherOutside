...
extension ForecastApiManager: ForecastApiManagerProtocol {
    ...
    private func saveData(_ forecast: ForecastResponse) {
        dataManager.addMetaInfo(forecast.timezone, forecast.lat, forecast.lon)
        guard let metaInfo = self.dataManager.metaInfo.last else {
            return
        }
        
        self.dataManager.addCurrent(forecast.current, for: metaInfo)
        for dailyForecast in forecast.daily {
            self.dataManager.addDaily(dailyForecast, for: metaInfo)
        }
        for hourlyForecast in forecast.hourly {
            self.dataManager.addHourly(hourlyForecast, for: metaInfo)
        }
    }
}
